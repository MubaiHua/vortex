...
void LsuUnit::tick() {
	...
	for (uint32_t iw = 0; iw < ISSUE_WIDTH; ++iw) {
		...
		// bool is_write = ((trace->lsu_type == LsuType::STORE) || (trace->lsu_type == LsuType::TCU_STORE) || (trace->lsu_type == LsuType::PREFETCH)); 
		bool is_prefetch = (trace->lsu_type == LsuType::PREFETCH);
